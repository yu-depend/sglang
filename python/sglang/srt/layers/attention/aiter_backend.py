...
class AiterAttnBackend(AttentionBackend):
    def __init__(
        ...
        elif not self.use_mla and get_bool_env_var("SGLANG_AITER_UNIFIED_ATTN", "True"):
            self.use_triton_unified_attention = True
    ...
    def forward_decode(
        ...
        if save_kv_cache:
            if self.use_triton_unified_attention:
                ...
                swa_slot_mapping = None
                if layer.sliding_window_size > 0 and self.use_sliding_window_kv_pool:
                    swa_slot_mapping = token_to_kv_pool.full_to_swa_index_mapping.long()
